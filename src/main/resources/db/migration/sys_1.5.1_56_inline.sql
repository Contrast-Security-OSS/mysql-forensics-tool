...
SELECT "WARNING: Using a routine that could cause binary log events, but disabling of binary logging has been removed";
...
CREATE DEFINER=contrast TRIGGER sys_config_insert_set_user BEFORE INSERT on sys_config
    FOR EACH ROW
BEGIN
...
CREATE DEFINER=contrast TRIGGER sys_config_update_set_user BEFORE UPDATE on sys_config
    FOR EACH ROW
BEGIN
    IF @sys.ignore_sys_config_triggers != true AND NEW.set_by IS NULL THEN
        SET NEW.set_by = USER();
...
CREATE DEFINER=contrast FUNCTION extract_schema_from_file_name (
        path VARCHAR(512)
    )
    RETURNS VARCHAR(64) 
    COMMENT '
...
CREATE DEFINER=contrast FUNCTION extract_table_from_file_name (
        path VARCHAR(512)
    )
    RETURNS VARCHAR(64) 
    COMMENT '
...
CREATE DEFINER=contrast FUNCTION format_bytes (
...
CREATE DEFINER=contrast FUNCTION format_path (
        in_path VARCHAR(512)
    )
...
CREATE DEFINER=contrast FUNCTION format_statement (
        statement LONGTEXT
    )
    RETURNS LONGTEXT
    COMMENT '
...
CREATE DEFINER=contrast FUNCTION format_time (
        -- We feed in and return TEXT here, as aggregates of
...
CREATE DEFINER=contrast FUNCTION list_add (
        in_list TEXT,
        in_add_value TEXT
    )
    RETURNS TEXT
...
CREATE DEFINER=contrast FUNCTION list_drop (
        in_list TEXT,
        in_drop_value TEXT
    )
    RETURNS TEXT
...
CREATE DEFINER=contrast FUNCTION ps_is_account_enabled (
        in_host VARCHAR(60), 
        in_user VARCHAR(16)
    ) 
    RETURNS ENUM('YES', 'NO')
...
CREATE DEFINER=contrast FUNCTION ps_is_consumer_enabled (
...
CREATE DEFINER=contrast FUNCTION ps_is_instrument_default_enabled (
...
CREATE DEFINER=contrast FUNCTION ps_is_instrument_default_timed (
        in_instrument VARCHAR(128)
    ) 
    RETURNS ENUM('YES', 'NO')
    COMMENT '
...
CREATE DEFINER=contrast FUNCTION ps_is_thread_instrumented (
...
CREATE DEFINER=contrast FUNCTION ps_thread_id (
        in_connection_id BIGINT UNSIGNED
...
CREATE DEFINER=contrast FUNCTION ps_thread_account (
...
CREATE DEFINER=contrast FUNCTION ps_thread_stack (
...
CREATE DEFINER=contrast FUNCTION quote_identifier(in_identifier TEXT)
    RETURNS TEXT CHARSET UTF8
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast FUNCTION sys_get_config (
...
CREATE DEFINER=contrast FUNCTION version_major ()
...
CREATE DEFINER=contrast FUNCTION version_minor ()
    RETURNS TINYINT UNSIGNED
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast FUNCTION version_patch ()
    RETURNS TINYINT UNSIGNED
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast PROCEDURE create_synonym_db (
...
CREATE DEFINER=contrast PROCEDURE execute_prepared_stmt (
...
CREATE DEFINER=contrast PROCEDURE diagnostics (
...
CREATE DEFINER=contrast PROCEDURE ps_statement_avg_latency_histogram ()
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast PROCEDURE ps_trace_statement_digest (
...
DELIMITER $$
CREATE DEFINER=contrast PROCEDURE ps_trace_thread (
...
CREATE DEFINER=contrast PROCEDURE ps_setup_disable_background_threads ()
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast PROCEDURE ps_setup_disable_consumer (
...
CREATE DEFINER=contrast PROCEDURE ps_setup_disable_instrument (
...
CREATE DEFINER=contrast PROCEDURE ps_setup_disable_thread (
...
CREATE DEFINER=contrast PROCEDURE ps_setup_enable_background_threads ()
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast PROCEDURE ps_setup_enable_consumer (
        IN consumer VARCHAR(128)
    )
    COMMENT '
             Description
...
CREATE DEFINER=contrast PROCEDURE ps_setup_enable_instrument (
        IN in_pattern VARCHAR(128)
    )
    COMMENT '
             Description
...
CREATE DEFINER=contrast PROCEDURE ps_setup_enable_thread (
        IN in_connection_id BIGINT
    )
    COMMENT '
             Description
...
CREATE DEFINER=contrast PROCEDURE ps_setup_reload_saved ()
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast PROCEDURE ps_setup_reset_to_default (
...
CREATE DEFINER=contrast PROCEDURE ps_setup_save (
        IN in_timeout INT
    )
    COMMENT '
             Description
...
CREATE DEFINER=contrast PROCEDURE ps_setup_show_disabled (
...
CREATE DEFINER=contrast PROCEDURE ps_setup_show_disabled_consumers ()
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast PROCEDURE ps_setup_show_disabled_instruments ()
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast PROCEDURE ps_setup_show_enabled (
        IN in_show_instruments BOOLEAN,
        IN in_show_threads BOOLEAN
    )
    COMMENT '
...
CREATE DEFINER=contrast PROCEDURE ps_setup_show_enabled_consumers ()
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast PROCEDURE ps_setup_show_enabled_instruments ()
    COMMENT '
             Description
             -----------
...
CREATE DEFINER=contrast PROCEDURE ps_truncate_all_tables (
        IN in_verbose BOOLEAN
    )
    COMMENT '
             Description
...
CREATE DEFINER=contrast PROCEDURE statement_performance_analyzer (
...
CREATE DEFINER=contrast PROCEDURE table_exists (
